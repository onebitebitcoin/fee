...
def find_cheapest_path_from_snapshot_rows(
    ...
    # 글로벌 거래소 Lightning Network 출금 행 (swap 유무 무관하게 공통 사용)
    ...
    # Lightning exit 경로 추가 (ln_to_onchain 스왑 포함)
    if lightning_swap_rows:
        # buy 모드: ln_to_onchain 서비스만 사용 (BitFreezer 등)
        # 흐름: 한국 → 글로벌 거래소 BTC Lightning 출금 → ln_to_onchain 스왑 → 개인 on-chain 지갑
    ...
    # 직접 LN 출금 경로 (스왑 없음): 글로벌 거래소 LN 출금 → 개인 Lightning 지갑
    # '__direct__' 센티널: 스왑 서비스 없이 개인 LN 지갑으로 직접 수령
    if global_ln_wd_fee is not None:
        ln_network_label = global_ln_wd_row.network_label if global_ln_wd_row else 'Lightning Network'
        ...
        for exchange in GROUPS['korea']:
            ...
            for row in ctx.withdrawals_by_key.get((exchange, 'USDT'), []):
                ...
                if global_usdt_nets and normalize_usdt_network(row.network_label) not in global_usdt_nets:
                    continue
                if is_suspended(ctx.maintenance_status, exchange, 'USDT', row.network_label):
                    continue
                ...
                usdt_for_btc = usdt_after_wd - global_trading_fee_usdt
                btc_at_global = usdt_for_btc / ctx.global_btc_price_usd
                btc_received = btc_at_global - global_ln_wd_fee
                if btc_received <= 0:
                    continue
                ...
                global_trading_fee_krw = round(global_trading_fee_usdt * ctx.usd_krw_rate)
                total_fee_krw = trading_fee_krw + withdrawal_fee_krw + global_trading_fee_krw + global_ln_wd_fee_krw
                ...
                paths.append({
                    ...
                    'breakdown': {
                        'components': [
                            fee_component('국내 매수 수수료', trading_fee_krw, rate_pct=korean_taker * 100, is_fixed=False),
                            fee_component(
                                'USDT 출금 수수료', withdrawal_fee_krw,
                                amount_text=f'{row.fee:g} USDT',
                                source_url=get_withdrawal_source_url(exchange, 'USDT', row.network_label),
                                is_fixed=True,
                            ),
                            fee_component(
                                '해외 BTC 매수 수수료', global_trading_fee_krw,
                                rate_pct=ctx.global_taker * 100,
                                amount_text=f'{round(global_trading_fee_usdt, 8)} USDT',
                                is_fixed=False,
                            ),
        ...
        for exchange in GROUPS['korea']:
            ...
            for row in ctx.withdrawals_by_key.get((exchange, 'BTC'), []):
                ...
                btc_received = btc_after_domestic_wd - global_ln_wd_fee
                if btc_received <= 0:
                    continue
                ...
                total_fee_krw = trading_fee_krw + domestic_wd_fee_krw + global_ln_wd_fee_krw
