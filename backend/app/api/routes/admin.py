...
DEFAULT_REGISTRY: dict = {
    'domestic': {
        'upbit': [
            {'label': '출금 주소 사전 등록 필수', 'desc': 'My Wallet에 출금 주소를 미리 등록해야 합니다.', 'level': 'required', 'condition': None, 'source': '업비트 고객센터 / 금융정보분석원(FIU) 가이드라인'},
            {'label': '본인 명의 지갑만 허용', 'desc': '타인 명의 거래소 주소 또는 서비스 주소로 출금이 불가합니다.', 'level': 'required', 'condition': None, 'source': '특금법 제7조 (자금세탁방지)'},
        ...
        'bithumb': [
            {'label': '출금 주소 사전 등록 필수', 'desc': '출금 주소를 주소록에 미리 등록해야 합니다. 2025년 4월부터 100만원 미만 출금도 주소 사전 등록이 필요합니다.', 'level': 'required', 'condition': None, 'source': '빗썸 공지사항 2025.04 / 금융정보분석원 가이드라인'},
            {'label': 'KYC 실명 인증 필수', 'desc': '본인 인증이 완료된 계정에서만 출금 가능합니다.', 'level': 'required', 'condition': None, 'source': '특금법 제5조 (고객 확인 의무)'},
        ...
        'korbit': [
            {'label': '출금 주소 사전 등록 필수', 'desc': '100만원 이상 개인지갑 출금 시 주소를 미리 등록해야 합니다. 100만원 미만은 자유 출금 가능합니다.', 'level': 'required', 'condition': None, 'source': '코빗 고객센터 / 금융정보분석원 가이드라인'},
            {'label': 'KYC 실명 인증 필수', 'desc': '본인 인증 완료 필요합니다.', 'level': 'required', 'condition': None, 'source': '특금법 제5조 (고객 확인 의무)'},
        ...
        'coinone': [
            {'label': '출금 주소 사전 등록 필수', 'desc': '안심 주소록에 출금 주소를 등록해야 합니다. 담당자 심사 후 승인되며 수일이 소요될 수 있습니다.', 'level': 'required', 'condition': None, 'source': '코인원 고객센터 / 금융정보분석원 가이드라인'},
        ...
        'gopax': [
            {'label': '출금 주소 사전 등록 필수', 'desc': '출금 주소를 미리 등록해야 합니다. 관리자 증빙 심사 후 승인됩니다.', 'level': 'required', 'condition': None, 'source': '고팍스 고객센터 / 금융정보분석원 가이드라인'},
            {'label': 'KYC 실명 인증 필수', 'desc': '본인 인증이 완료된 계정에서만 출금 가능합니다.', 'level': 'required', 'condition': None, 'source': '특금법 제5조 (고객 확인 의무)'},
    ...
    'global': {
        'binance': [
            {'label': 'KYC 인증 (Level 1 이상)', 'desc': '신분증 인증이 완료되어야 입출금이 가능합니다. 미인증 시 출금 불가.', 'level': 'required', 'condition': None, 'source': 'Binance Terms of Service / FATF 권고안'},
            {'label': '일일 출금 한도', 'desc': '미인증: 0.06 BTC/day, Basic 인증(Level 1): 0.6 BTC/day, 고급 인증(Level 2): 100 BTC/day', 'level': 'info', 'condition': None, 'source': 'Binance Help Center — Updates to Daily Withdrawal Limits'},
        ...
        'okx': [
            {'label': 'KYC 인증 필수', 'desc': '개인 신원 인증 완료 필요합니다.', 'level': 'required', 'condition': None, 'source': 'OKX Terms of Service / FATF 권고안'},
            {'label': '일일 출금 한도', 'desc': 'KYC1(기본·이메일 인증): $5,000/day, KYC2(신분증+얼굴 인증): $500,000/day, KYC3(고급): 무제한', 'level': 'info', 'condition': None, 'source': 'OKX Help Center — KYC Requirements Guide (2025)'},
            {'label': '신규 주소 24시간 지연', 'desc': '새로 등록한 주소는 24시간 후 출금이 가능합니다.', 'level': 'conditional', 'condition': '신규 주소 등록 시', 'source': 'OKX 보안 정책'},
        ],
        'bybit': [
            {'label': 'KYC 인증 필수', 'desc': '거주 국가 및 신분 인증 필요합니다.', 'level': 'required', 'condition': None, 'source': 'Bybit Terms of Service / FATF 권고안'},
            ...
            {'label': '일일 출금 한도', 'desc': '미인증: 2 BTC/day, Standard KYC(Level 1): 50 BTC/day, Advanced KYC(Level 2): 100 BTC/day', 'level': 'info', 'condition': None, 'source': 'Bybit Help Center — FAQ Crypto Withdrawal'},
        ],
        'bitget': [
            {'label': 'KYC 인증 필수', 'desc': '신분증 인증이 필요합니다.', 'level': 'required', 'condition': None, 'source': 'Bitget Terms of Service / FATF 권고안'},
            {'label': '일일 출금 한도', 'desc': '미인증: $20,000/day, KYC 완료(VIP0): $3,000,000/day, 고급 VIP: 최대 $15,000,000/day', 'level': 'info', 'condition': None, 'source': 'Bitget — Mandatory KYC and Withdrawal Limit Policy'},
            {'label': '신규 주소 24시간 지연', 'desc': '새로 추가한 주소는 24시간 후 출금 가능합니다.', 'level': 'conditional', 'condition': '신규 주소 등록 시', 'source': 'Bitget 보안 정책'},
        ],
        'kraken': [
            {'label': 'KYC 인증 필수 (Intermediate 이상)', 'desc': '주소 및 신분 증명 완료 필요합니다.', 'level': 'required', 'condition': None, 'source': 'Kraken — Verification levels and limits / FATF 권고안'},
            {'label': '일일 출금 한도', 'desc': 'Intermediate: $500,000/day, Pro: 무제한', 'level': 'info', 'condition': None, 'source': 'Kraken Help Center — Deposit and Withdrawal Limits'},
        ],
        'coinbase': [
            {'label': 'KYC 인증 필수', 'desc': '신분 인증 및 거주지 인증 완료 필요합니다.', 'level': 'required', 'condition': None, 'source': 'Coinbase Terms of Service / FinCEN 규정'},
            {'label': '한국 법정화폐 서비스 미제공', 'desc': 'Coinbase는 한국 거주자에게 법정화폐 입출금 서비스를 제공하지 않습니다. 크립토 간 거래는 제한적으로 가능합니다.', 'level': 'required', 'condition': None, 'source': 'Coinbase 지역 서비스 정책'},
            {'label': '일일 출금 한도', 'desc': '계정 등급에 따라 한도 상이', 'level': 'info', 'condition': None, 'source': 'Coinbase Help Center'},
        ],
        'gate': [
            {'label': 'KYC 인증 필수', 'desc': '신분증 인증이 필요합니다. KYC 미완료 시 출금 불가.', 'level': 'required', 'condition': None, 'source': 'Gate.io Terms of Service / FATF 권고안'},
            {'label': '트래블룰 지원 (VerifyVASP·CODE)', 'desc': 'Gate.io는 VerifyVASP 및 CODE 트래블룰 솔루션을 지원합니다. 국내 거래소와의 이전이 가능합니다.', 'level': 'info', 'condition': None, 'source': 'FATF Recommendation 16 / Gate.io 공식 발표'},
            {'label': '신규 주소 24시간 지연', 'desc': '새로 추가한 출금 주소는 24시간 후 사용 가능합니다.', 'level': 'conditional', 'condition': '신규 주소 등록 시', 'source': 'Gate.io 보안 정책'},
            {'label': '일일 출금 한도', 'desc': 'KYC Lv1: $2,000,000/day, Lv2: $5,000,000/day', 'level': 'info', 'condition': None, 'source': 'Gate.io Help Center — Withdrawal Limits'},
        ],
    },
    'onchain': [
        {'label': '주소 오입력 시 복구 불가', 'desc': 'Bitcoin 블록체인 트랜잭션은 한번 전송되면 취소하거나 되돌릴 수 없습니다. 반드시 주소를 확인하세요.', 'level': 'required', 'condition': None, 'source': 'Bitcoin 프로토콜 (블록체인 불변성)'},
        {'label': '네트워크 수수료(Network Fee) 발생', 'desc': '네트워크 혼잡도에 따라 네트워크 수수료가 변동됩니다. 혼잡 시 수수료가 높아질 수 있습니다.', 'level': 'info', 'condition': None, 'source': 'Bitcoin 블록체인 프로토콜'},
        {'label': '입금 확인 시간 소요', 'desc': '1 블록 확인에 약 10분, 거래소 입금 반영까지 1~6 블록(10분~1시간) 소요됩니다.', 'level': 'info', 'condition': None, 'source': 'Bitcoin 프로토콜 (평균 블록 생성 10분)'},
    ],
}
