...
| `src/pages/explorer/ExplorerContext.tsx` | **상태·파생·핸들러 허브.** `useExplorerValue()`에 모든 useState/useMemo + `handleSearch`/`handleBack`/`handleNext`/`reset`. `ExplorerProvider`/`useExplorer()` 제공. 타입은 `ReturnType<typeof useExplorerValue>` 추론. |
